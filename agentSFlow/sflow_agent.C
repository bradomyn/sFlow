...
/* $Header: /root/InMon_Agent-5.7/RCS/sflow_agent.C,v 1.10 2006/07/10 18:20:22 root Exp root $ */
...
static void sfl_agent_jumpTablePossiblyAdd(SFLAgent *agent, SFLSampler *sampler);
...
SFLSampler *sfl_agent_addSampler(SFLAgent *agent, SFLDataSource_instance *pdsi)
{
  ...
  sfl_agent_jumpTablePossiblyAdd(agent, newsm);
...
int sfl_agent_removeSampler(SFLAgent *agent, SFLDataSource_instance *pdsi)
{
  ...
  for(SFLSampler *prev = NULL, *sm = agent->samplers; sm != NULL; prev = sm, sm = sm->nxt) {
    if(sfl_dsi_compare(pdsi, &sm->dsi) == 0) {
      ...
      if(sm->nxt) {
	/* If the next sampler is another instance for the same interface,
	   then it will probably need to take over as the jump-table target.
	   Thank you to CWChung for spotting this. */
	sfl_agent_jumpTablePossiblyAdd(agent, sm->nxt);
      }
      sflFree(agent, sm);
      return 1;
    }
...
/*_________________----------------------------------__________________
  _________________  sfl_agent_jumpTablePossiblyAdd  __________________
...
{
  // see if we should go in the ifIndex jumpTable
  if(SFL_DS_CLASS(sampler->dsi) == 0) {
    SFLSampler *test = sfl_agent_getSamplerByIfIndex(agent, SFL_DS_INDEX(sampler->dsi));
    if(test && (SFL_DS_INSTANCE(sampler->dsi) < SFL_DS_INSTANCE(test->dsi))) {
      // replace with this new one because it has a lower ds_instance number
      sfl_agent_jumpTableRemove(agent, test);
      test = NULL;
    }
    if(test == NULL) sfl_agent_jumpTableAdd(agent, sampler);
  }
}
